...
class Aiko {
    ...
    async auth() {
        this.token = await axios.get(
            this.url + 'auth/access_token?user_id=' + this.aikoLogin + '&user_secret=' + this.aikoPassword
        )
        if (!this.token) {
    ...
    async uploadCategory() {
        const nomenclature = await axios.get(
            this.url + 'nomenclature/' + this.org.data[0].id + '?access_token=' + this.token.data
        )
        if (!nomenclature) {
    ...
    async sendOrder(order) {
        ...
        const payment = await axios.get(
            this.url +
                'rmsSettings/getPaymentTypes?access_token=' +
                this.token.data +
                '&organization=' +
                this.org.data[0].id
        )
        ...
        if (matches && matches[0]) {
            ...
            await axios
                .post(
                    this.url + 'orders/add?access_token=' + this.token.data + '&organization=' + this.org.data[0].id,
                    JSON.stringify({
